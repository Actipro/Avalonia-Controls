...
Use [Foreground](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Foreground) and the standard font properties, such as [FontFamily](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.FontFamily), [FontSize](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.FontSize), and [FontWeight](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.FontWeight), to style the text. The control measures and presents one line.
...
The [TransitionDirection](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.TransitionDirection) property defaults to [Automatic](xref:@ActiproUIRoot.Controls.AnimatedTextPresenterTransitionDirection.Automatic). Each recognized numeric token follows its own value trend: increasing numbers roll up and decreasing numbers roll down. Other changed content rolls up.
...
Set Avalonia's `FlowDirection` property for right-to-left content. Static and animated text use the same complete formatted endpoints so signs, affixes, and mixed-direction glyphs remain in their expected visual order.

## Foreground Transitions

Replacing [Foreground](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Foreground) automatically includes the old and new brushes in the active presentation transition. A foreground-only change keeps the text stationary and does not change the presenter's desired size. When [Text](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Text) and [Foreground](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Foreground) change in the same UI update, the original text and brush transition directly to the latest values regardless of property notification order.
...
[Text](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Text) and [Foreground](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Foreground) always contain the current target presentation, even while older content remains visible during a transition. Multiple changes in the same update preserve the original source and latest target. If either property changes after a transition is underway, the active transition is replaced and the previous target presentation becomes the source for the newest values.

No transition starts while the control is unloaded, effectively hidden, disabled by animation policy, or using a zero effective duration. If it becomes effectively hidden during a transition, transient animation content is discarded and the current [Text](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Text) and [Foreground](xref:@ActiproUIRoot.Controls.AnimatedTextPresenter.Foreground) presentation is used when it becomes visible again.
