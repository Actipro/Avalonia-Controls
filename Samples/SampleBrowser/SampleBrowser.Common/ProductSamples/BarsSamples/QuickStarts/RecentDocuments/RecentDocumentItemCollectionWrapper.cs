...
public class RecentDocumentItemCollectionWrapper<TSource> : IList, ICollection<RecentDocumentItem>, INotifyCollectionChanged {
	...
	private readonly List<Tuple<TSource, RecentDocumentItem>> _items = [];
	...
	void ICollection<RecentDocumentItem>.Add(RecentDocumentItem item)
		=> throw new NotSupportedException();

	void ICollection<RecentDocumentItem>.Clear()
		=> throw new NotSupportedException();
	...
	int ICollection<RecentDocumentItem>.Count
		=> ((ICollection)this).Count;

	bool ICollection<RecentDocumentItem>.IsReadOnly
		=> true;

	bool ICollection<RecentDocumentItem>.Remove(RecentDocumentItem item)
		=> throw new NotSupportedException();
	...
	int IList.IndexOf(object? value) {
		if (value is RecentDocumentItem) {
	...
	private Tuple<TSource, RecentDocumentItem> AddWrappedItem(TSource source) {
		...
		if (source is INotifyPropertyChanged observableSource)
			observableSource.PropertyChanged += OnSourcePropertyChanged;
			...
			case NotifyCollectionChangedAction.Replace:
				...
				if (e.OldItems is not null) {
					foreach (var source in e.OldItems.OfType<TSource>()) {
						if (RemoveWrappedItem(source) is { } tuple)
				...
				if (e.NewItems is not null) {
					foreach (var source in e.NewItems.OfType<TSource>()) {
						if (AddWrappedItem(source) is { } tuple)
	...
	private Tuple<TSource, RecentDocumentItem>? RemoveWrappedItem(TSource source) {
		...
		if (tuple is not null) {
			...
			if (tuple.Item1 is INotifyPropertyChanged observableSource)
				observableSource.PropertyChanged -= OnSourcePropertyChanged;
			tuple.Item2.PropertyChanged -= OnItemPropertyChanged;
