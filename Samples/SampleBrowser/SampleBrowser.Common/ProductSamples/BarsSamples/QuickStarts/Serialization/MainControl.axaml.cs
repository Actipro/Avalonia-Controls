...
public partial class MainControl : UserControl {
	...
	private void OnQuickAccessToolBarItemAdding(object? sender, RibbonQuickAccessToolBarItemAddingEventArgs e) {
		...
		if (!e.Cancel) {
			...
			if ((e is { Item: null, Key: { } key }) && _mvvmControls.TryGetValue(key, out var item))
				e.Item = item;
	...
	private bool TryRestoreLayout(Ribbon ribbon, string layout) {
		try {
			...
			var options = OptionsModel.Options;
	...
	/// <summary>
	/// The command to restore the configured layout for the MVVM sample.
	/// </summary>
	public ICommand MvvmRestoreLayoutCommand {
		get {
			if (_mvvmRestoreLayoutCommand is null) {
				_mvvmRestoreLayoutCommand = new DelegateCommand<object>(_ => {
					var currentLayout = CurrentMvvmLayout;
	...
	/// <summary>
	/// The command to restore the original layout for the MVVM sample.
	/// </summary>
	public ICommand MvvmRestoreOriginalCommand {
		get {
			if (_mvvmRestoreOriginalCommand is null) {
				_mvvmRestoreOriginalCommand = new DelegateCommand<object>(_ => {
	...
	/// <summary>
	/// The command to restore the configured layout for the XAML sample.
	/// </summary>
	public ICommand XamlRestoreLayoutCommand {
		get {
			if (_xamlRestoreLayoutCommand is null) {
				_xamlRestoreLayoutCommand = new DelegateCommand<object>(_ => {
					var currentLayout = CurrentXamlLayout;
					if (string.IsNullOrEmpty(currentLayout)) {
						MessageBox.Show("The current layout is undefined and cannot be restored.", "Restore", MessageBoxButtons.OK, MessageBoxImage.Error);
						return;
					}
	...
	/// <summary>
	/// The command to restore the original layout for the XAML sample.
	/// </summary>
	public ICommand XamlRestoreOriginalCommand {
		get {
			if (_xamlRestoreOriginalCommand is null) {
				_xamlRestoreOriginalCommand = new DelegateCommand<object>(_ => {
	...
	/// <summary>
	/// The command to save the current layout for the XAML sample.
	/// </summary>
	public ICommand XamlSaveLayoutCommand {
		get {
			if (_xamlSaveLayoutCommand is null) {
				_xamlSaveLayoutCommand = new DelegateCommand<object>(_ => {
					// Attempt to save the current layout
					if (TrySaveLayout(xamlRibbon, out var layout))
						CurrentXamlLayout = layout;
				});
			}
