...
partial class BarManager {
	...
	/// <summary>
	/// Creates an image based on the given options, or <c>null</c> if a corresponding image is not available.
	/// </summary>
	...
	/// <summary>
	/// Creates an image based on the given options with a color bar overlay.
	/// </summary>
	/// <inheritdoc cref="CreateBitmapImage(BarImageOptions, string, string, string)"/>
	...
	private static IImage? CreateBitmapImageWithColorBar(BarImageOptions options, Color defaultColor, string? smallFileName, string? mediumFileName, string? largeFileName) {
		// Load the base image
		var image = CreateBitmapImage(options, smallFileName, mediumFileName, largeFileName);
		if (image is not null) {
			...
			if ((imageHeight > 0) && (imageWidth > 0)) {
				...
				if ((colorBarBounds.Height > 0) && (colorBarBounds.Width > 0)) {
					...
					image = UI.Avalonia.Media.ImageProvider.GetImageSourceWithColorSwatch(image, colorBarBounds, options.ContextualColor.GetValueOrDefault(defaultColor));
				}
			}
