...
public class TextBoxBarManagerCommandAdapter {
	...
	/// <param name="textBox">The <c>TextBox</c> control to be adapted.</param>
	...
	private ICommand CopyCommand {
		get => _copyCommand ??= new DelegateCommand<object?>(
			_ => TextBox.Copy(),
			_ => TextBox.CanCopy
		);
	}
	...
	private ICommand CutCommand {
		get => _cutCommand ??= new DelegateCommand<object?>(
			_ => TextBox.Cut(),
			_ => TextBox.CanCut
		);
	}
	...
	private ICommand InsertSymbolCommand {
		get => _insertSymbolCommand ??= new DelegateCommand<SymbolBarGalleryItemViewModel>(p => {
	...
	private ICommand PasteCommand {
		get => _pasteCommand ??= new DelegateCommand<object?>(
			_ => TextBox.Paste(),
			_ => TextBox.CanPaste
		);
	}
	...
	private ICommand RedoCommand {
		get => _redoCommand ??= new DelegateCommand<object?>(
			_ => TextBox.Redo(),
			_ => TextBox.CanRedo
		);
	}
	...
	private ICommand SetFontColorCommand {
		get => _setFontColorCommand ??= new PreviewableDelegateCommand<ColorBarGalleryItemViewModel>(
			executeAction: param => {
				...
				UpdateCurrentTextStyle(x => x.FontColor = param?.Value ?? DefaultFontForegroundPickerColor);

				if (BarManager.ControlViewModels[BarControlKeys.FontColor] is BarSplitButtonViewModel buttonViewModel) {
					buttonViewModel.CommandParameter = param;
	...
	private ICommand SetTextAlignmentCommand {
		get => _setTextAlignmentCommand ??= new DelegateCommand<TextAlignment>(param => {
	...
	private ICommand SetTextHighlightColorCommand {
		get => _setTextHighlightColorCommand ??= new PreviewableDelegateCommand<ColorBarGalleryItemViewModel>(
			executeAction: param => {
				OnRequestSaveAndExitPreviewMode();
				UpdateCurrentTextStyle(x => x.TextHighlightColor = param?.Value ?? DefaultFontBackgroundPickerColor);
	...
	private ICommand UndoCommand {
		get => _undoCommand ??= new DelegateCommand<object?>(
			_ => TextBox.Undo(),
			_ => TextBox.CanUndo
		);
	}
	...
	private ICommand UnknownFontSizeCommand {
		get => _unknownFontSizeCommand ??= new DelegateCommand<string?>(
			executeAction: param => {
				if (int.TryParse(param, out var fontSize))
					UpdateCurrentTextStyle(x => x.FontSize = fontSize);
			},
			canExecuteFunc: param => {
				return int.TryParse(param, out var fontSize)
					&& byte.MinValue <= fontSize
					&& fontSize <= byte.MaxValue;
			}
		);
	}
	...
	private void UpdateBarControlViewModelsFromCurrentStyle(RichTextStyle? textStyle = null) {
		...
		if (textAlignment is not (TextAlignment.Center or TextAlignment.Justify or TextAlignment.Right)) {
			// Treat all other values like left alignment for this sample
