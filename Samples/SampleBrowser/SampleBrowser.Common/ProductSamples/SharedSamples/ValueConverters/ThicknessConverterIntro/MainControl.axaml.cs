...
public partial class MainControl : UserControl {
	...
	public MainControl() {
		...
		filterLeftCheckBox.IsCheckedChanged += OnFilterCheckBoxIsCheckedChanged;
		filterTopCheckBox.IsCheckedChanged += OnFilterCheckBoxIsCheckedChanged;
		filterRightCheckBox.IsCheckedChanged += OnFilterCheckBoxIsCheckedChanged;
		filterBottomCheckBox.IsCheckedChanged += OnFilterCheckBoxIsCheckedChanged;
	...
	private void ConfigureBinding(ThicknessConverter? converter = null) {
		...
		_clearBinding = basicSampleBorder.BindToProperty(Border.BorderThicknessProperty, basicSample, TextBox.TextProperty, BindingMode.OneWay, converter);
	}
	...
	private ThicknessConverter? GetConverter() {
		if (
			this.TryFindResource(nameof(ThicknessConverter), out var resource)
	...
	private void OnFilterCheckBoxIsCheckedChanged(object? sender, RoutedEventArgs e) {
		if (GetConverter() is { } converter) {
			converter.Filter = GetFilterSides();
			ConfigureBinding(converter);

			FilterSampleCode = (converter.Filter == Sides.All) ? string.Empty : converter.Filter.ToString();
		}
	}
