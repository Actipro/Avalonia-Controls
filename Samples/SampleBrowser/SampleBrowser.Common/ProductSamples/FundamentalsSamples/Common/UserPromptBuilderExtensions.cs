...
public static class UserPromptBuilderExtensions {
	...
	public static UserPromptBuilder WithStatusIconTheme(this UserPromptBuilder builder, MessageBoxImage? statusIcon = null) {
		return builder
			.AfterBuild(_ => {
				...
				Hue? hue = statusIcon switch {
					MessageBoxImage.Error => Hue.Red,
					MessageBoxImage.Information => Hue.Sky,
					MessageBoxImage.Warning => Hue.Orange,
					MessageBoxImage.Question => Hue.Indigo,
					_ => null
				};

				// Quit if there is not a hue to apply
				if (hue is null)
					return;
				...
				var colorRamp = colorPalette.Ramps[hue.Value] ?? throw new InvalidOperationException("Unable to resolve the color ramp.");
	...
	public static UserPromptButtonBuilder WithIcon(this UserPromptButtonBuilder builder, IImage? image, HorizontalAlignment imageAlignment) {
		return builder.AfterBuild(_ => {
			if ((image is not null) && (builder.Instance is Button button)) {

				var existingContent = button.Content as Control;
				if ((existingContent is null) && (button.Content is string label))
