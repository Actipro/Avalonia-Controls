...
/// </summary>
[TemplatePart(CodeExamplePanelPartName, typeof(Panel), IsRequired = true)]
...
public class ControlExample : HeaderedContentControl {
	...
	public ControlExample() {
		CodeExamples.CollectionChanged += (_, _) => OnCodeExamplesCollectionChanged();
	...
	private void BringCodeExamplesIntoView() {
		if (
			CodeExamplePanel is { } codeExamplePanel
			&& codeExamplePanel.FindDescendantOfType<ItemsControl>() is { } itemsControl
			&& this.FindAncestorOfType<ScrollViewer>() is { } scrollViewer
			&& codeExamplePanel.TransformToVisual(scrollViewer) is Matrix transform
		) {
			...
			var targetBounds = new Rect(
				...
				bottomRight: transform.Transform(new Point(0.0, Math.Min(MaxCodeExampleReveal, itemsControl.DesiredSize.Height)))
			);

			// Determine the amount to scroll to try and bring the target bounds fully into view and scroll as needed
			var deltaY = Math.Min(targetBounds.Top - scrollViewer.Offset.Y, targetBounds.Bottom - viewport.Height).ClampToNonnegative();
			if (deltaY > 0.0)
	...
	private static CodeExampleKind CoerceSelectedCodeExampleKindProperty(AvaloniaObject obj, CodeExampleKind kind) {
		if ((obj is ControlExample { IsCodeExampleKindSelectorVisible : true }) && (kind == CodeExampleKind.Unspecified)) {
	...
	private void UpdateCodeExampleVisibility() {
		foreach (var codeExample in CodeExamples) {
			if (codeExample is { Kind: not CodeExampleKind.Unspecified })
	...
	private void UpdateHasCode()
		=> HasCode = CodeExamples.Any(x => x?.IsVisible == true);
	...
	/// </summary>
	public ObservableCollection<CodeExample?> CodeExamples { get; } = [];
	...
	/// <summary>
	/// Indicates if teh code example kind selector control is visible.
	/// </summary>
	/// <value>
