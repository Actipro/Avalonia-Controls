...
public class DocumentViewer : ContentControl {
	...
	public static readonly StyledProperty<Thickness> DocumentMarginProperty
		= AvaloniaProperty.Register<DocumentViewer, Thickness>(nameof(DocumentMargin), defaultValue: new Thickness(50, 50, 50, 30));

	/// <summary>
	/// Defines the <see cref="MaxDocumentWidth"/> property.
	/// </summary>
	public static readonly StyledProperty<double> MaxDocumentWidthProperty
		= AvaloniaProperty.Register<DocumentViewer, double>(nameof(MaxDocumentWidth), defaultValue: 1100.0);
	...
	public static DocumentViewer? CreateForMarkdown(string markdownFilePath) {
		...
		if (!string.IsNullOrEmpty(markdown)) {
			...
			var content = new MarkdownDocumentConverter().Convert(markdown, typeof(Control), parameter: null, CultureInfo.CurrentCulture) as Control;
