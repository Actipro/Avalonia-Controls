...
public static class CodeTextBlockProperties {
	...
	private static readonly string[] _supportedLanguages = [CSharpLanguageName, XamlLanguageName, XmlLanguageName];
	...
	private abstract class ParserBase {
		...
		public ParserBase(string text) {
			_text = string.IsNullOrWhiteSpace(text)
				? string.Empty
				: text;
		...
		protected string? GetText(int maxLength) {
			if (0 <= Position && Position < _textLength) {
				var endPosition = Math.Min(_textLength, Position + maxLength);
				var length = endPosition - Position;
				try {
		...
		protected int Position {
			get => _position;
			set => _position = value.ClampToRange(0, _textLength);
		}
	...
	private class XmlInlineParser : ParserBase {
		...
		public override IEnumerable<Inline> Parse() {
			...
			var tokenText = new StringBuilder();
			Brush? tokenBrush = null;
			var isWithinAttributedTag = false;
