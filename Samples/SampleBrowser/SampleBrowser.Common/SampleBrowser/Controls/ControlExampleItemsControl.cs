...
public class ControlExampleItemsControl : HeaderedItemsControl {
	...
	public ControlExampleItemsControl() {
		Documentation.CollectionChanged += (_, _) => UpdateHasDocumentation();
		Items.CollectionChanged += OnItemsCollectionChanged;
		RelatedSamples.CollectionChanged += (_, _) => UpdateHasRelatedSamples();
	}
	...
	private void UpdateHasDocumentation()
		=> HasDocumentation = (Documentation.Any(x => x is not null));

	private void UpdateHasRelatedSamples()
		=> HasRelatedSamples = (RelatedSamples.Any(x => x is not null));
	...
	/// </summary>
	public ObservableCollection<ControlExampleLinkItem> Documentation { get; } = [];
	...
	/// </summary>
	public ObservableCollection<ControlExampleLinkItem> RelatedSamples { get; } = [];

	/// <summary>
	/// The table of contents.
	/// </summary>
	public ObservableCollection<ControlExampleTocItem> Toc { get; } = [];
	...
	public void Open() {
		if (!string.IsNullOrWhiteSpace(Url)) {
			if (Url.StartsWith("http", StringComparison.OrdinalIgnoreCase))
				ApplicationViewModel.Instance.OpenUrlCommand.Execute(Url);
			else
				ApplicationViewModel.Instance.NavigateViewToItemInfo(new Uri("https://ActiproSoftware" + Url));
		}
	}
