...
public static class ObservableStyledClassesProperties {
	...
	private static readonly HashSet<StyledElementClassesCollectionWatcher> _watchers = [];
	...
	/// <summary>
	/// Defines the <c>ClassesString</c> attached property.
	/// </summary>
	public static readonly AttachedProperty<string> ClassesStringProperty
		= AvaloniaProperty.RegisterAttached<StyledElement, string>("ClassesString", typeof(ObservableStyledClassesProperties), defaultValue: string.Empty, defaultBindingMode: BindingMode.OneWay);
	...
	private class StyledElementClassesCollectionWatcher : IDisposable {
		...
		public StyledElementClassesCollectionWatcher(StyledElement styledElement) {
			ArgumentNullException.ThrowIfNull(styledElement);
		...
		public bool IsWatching(StyledElement styledElement) {
			return StyledElementReference.TryGetTarget(out var target)
				&& ReferenceEquals(target, styledElement);
		}

		public bool TryGetStyledElement([NotNullWhen(returnValue: true)] out StyledElement? styledElement)
			=> StyledElementReference.TryGetTarget(out styledElement);
	...
	private static void OnIsWatchingChanged(AvaloniaObject obj, AvaloniaPropertyChangedEventArgs e) {
		if (obj is StyledElement styledElement) {
			var isWatching = e.GetNewValue<bool>();
			if (isWatching)
	...
	private static void OnWatchedElementClassesChanged(object? sender, EventArgs e) {
		if (
			sender is StyledElementClassesCollectionWatcher watcher
