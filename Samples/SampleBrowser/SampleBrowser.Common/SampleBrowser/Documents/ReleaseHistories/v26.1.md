...
- Added opacity animation when dragging non-hosted floating windows and using DockSite.NonHostedFloatingWindowDragOpacity. 
- Fixed issue with incorrect scaling of dock guides, dock previews, and non-hosted popups on macOS with HiDPI (Retina) scaling.
- Fixed DockSite.NonHostedFloatingWindowDragOpacity to only be applied after a drag threshold instead of immediately on pointer down.
## Fundamentals
...
- Updated CircularProgressBar logic for indeterminate mode.
## Themes
- Updated native control themes with changes in Avalonia through v12.0.5.
- Added the AnimationSettings.IsAnimationSupported property that can be used to determine if animations are supported on the current platform.  The new IsAnimationSupportedOverride property can be used to override the default behavior and force animations to be enabled or disabled.
- Fixed issue displaying AutoCompleteBox.PlaceholderText due to the PlaceholderForeground not having a default value.
- Fixed a SplitView layout issue related to compact left placement.
## Shared Library
- Fixed an issue introduced in a previous release where BorderChildClipConverter was not binding clip properties properly, which could affect controls like AvatarGroup.
## All
- Updated the license dialog logic to better handle licensed customers evaluating an unlicensed product.
