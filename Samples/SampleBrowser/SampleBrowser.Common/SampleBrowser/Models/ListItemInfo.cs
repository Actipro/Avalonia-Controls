...
public class ListItemInfo : ObservableObjectBase {
	...
	public ListItemInfo() {
		...
		if (Application.Current is { } application)
			application.ActualThemeVariantChanged += OnApplicationThemeVariantChanged;
	}
	...
	private void UpdateImageFromUri(Uri? imageUri) {
		if (imageUri is not null) {
			...
			if (altUri is not null) {
				try {
					ImageSource = new Bitmap(AssetLoader.Open(altUri));
					return;
				}
				catch { }
			}
			...
			// Load default image
			ImageSource = new Bitmap(AssetLoader.Open(imageUri));
		}
		else
			ImageSource = null;
	}
